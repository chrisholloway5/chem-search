#!/usr/bin/env python3
"""
Chem Search AI Assistant Script for Chem Search
Interfaces with ChemCrow to answer chemistry questions
"""
...
def ask_chemcrow(question):
    """Ask ChemCrow a chemistry question"""
    try:
        ...
        # Initialize ChemCrow with appropriate model
        try:
        ...
        result = {
            ...
            "model_used": "ChemCrow with gpt-3.5-turbo"
        }
        
        return result
        
    except ImportError as e:
        return {"error": f"ChemCrow not available: {str(e)}"}
    except Exception as e:
        error_msg = str(e)
        ...
        if "quota" in error_msg.lower() or "billing" in error_msg.lower():
            ...
            return {"error": f"ChemCrow query failed: {error_msg}"}

def main():
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python chemcrow_assistant.py <question>"}))
        sys.exit(1)
    
    question = sys.argv[1]
    result = ask_chemcrow(question)
